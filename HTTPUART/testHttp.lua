...
local function cbFnc(result,prompt,head,body)
    ...
    if result and body then
		...
		log.info("testHttp.cbFnc","Wait")
		log.info("testHttp.position",string.find(body,"PM2.5",1))
	end
	if result then
		i,j= string.find(body,"pm25",1)		--找到pm25的位置
		k,m = string.find(body,"pm10",1)	--找到pm10的位置
		log.info("testHttp.pmvalu,纬度 经度",string.sub(body,j+4,k-4),lat,lng,GetLoc)
	else
...
function getLocCb(result,lat,lng)
    ...
    if result==0 then
		...
		RecLng = lng
		GetLoc = 1
		log.info("testHttp",lat,lng,"start get weather");	--打印经度纬度，连接天气网站
		--http.request("GET","https://free-api.heweather.com/s6/air/now?".."location="..RecLng..","..Reclat.."&key=1fc52429a81741f586205f1ec307dcab",nil,nil,nil,nil,cbFnc)
		uart1Task.SendCmd(58)
    else
		GetLoc = 0
		uart1Task.SendCmd(58)
    end
		sys.timerStart(reqLbsLoc,1000)	--如果获取失败将会再2S后重新启动这个任务
end
...
--http.request("GET","https://free-api.heweather.com/s6/air/now?location=东莞&key=1fc52429a81741f586205f1ec307dcab",nil,nil,nil,nil,cbFnc)

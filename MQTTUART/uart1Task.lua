...
function SendCmd(s)
	local i=1
	local OpendCmd = {0xaa,0x06,0x61,0,0,0};
	local test1 = string.match(s,"p:on t:(%d+)")--取出只有数值部分数据
	--log.info("test1",test1)
	local opentime = tonumber(test1);			--将字符串转换成数值，这个函数找了好久才找到,tonumber太爽了
	--log.info("opentime",type(opentime),opentime)
	OpendCmd[4] = opentime/256
	OpendCmd[5] = opentime%256
	OpendCmd[6] = (OpendCmd[3] + OpendCmd[4] + OpendCmd[5])%256--一定要取余，不能像C语言一样可以8位自动取低位
	while true do
